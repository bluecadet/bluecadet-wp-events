(() => {
  ...
  // assets/classicEditor/src/events.js
  var ROOT_SELECTOR = ".bc-event-dates--classic";
  ...
  function syncRecurring(root) {
    ...
    const isRecurring2 = selectValue(valueInput) === "1";
    const wasRecurring = recurring.dataset.wasRecurring === "1";
    setHidden(recurring.querySelector(".bc-event-recurring__inner"), !isRecurring2);
    setHidden(recurring.querySelector(".js-bc-remove-recurring"), !(!isRecurring2 && wasRecurring));
  }
  ...
  function initPostPickers(scope) {
    scope.querySelectorAll(".js-bc-post-picker").forEach((select) => {
      ...
      new tom_select_complete_default(select, {
        valueField: "id",
        labelField: "title",
        searchField: "title",
        maxItems: 1,
  ...
  function validateTimestamp(root) {
    ...
    const missing = fields.filter(([, sel]) => selectValue(root.querySelector(sel)) === "").map(([label]) => label);
    if (missing.length) {
  ...
  function noticeHtml(innerHtml) {
    return `<div class="bce-events-validation-notice bce-events-validation-notice--error"><div class="bce-events-validation-notice__inner"><div class="bce-events-validation-notice__icon" aria-hidden="true">${VALIDATION_ICON}</div><div class="bce-events-validation-notice__content">${innerHtml}</div></div></div>`;
  }
  function runValidation(root) {
    ...
    if (missing.length) {
      notices.push(noticeHtml(
        `<p><strong>Required Fields are missing values:</strong></p><p>${escapeHtml(missing.join(", "))}</p>`
      ));
    }
    if (validateMissingStrategy(root)) {
      notices.push(noticeHtml(
        "<p><strong>Missing Recurring Strategy:</strong></p><p>A recurring event must have a frequency or specific dates defined.</p>"
      ));
    }
  ...
  function toTimestamp(date, time) {
    ...
    return fetch(`${url}/${restNamespace()}/to-timestamp?${params.toString()}`, {
      headers: nonce ? { "X-WP-Nonce": nonce } : {},
      credentials: "same-origin"
    }).then((res) => res.ok ? res.json() : null).then((data) => data && data.timestamp != null ? data.timestamp : null).catch(() => null);
  }
